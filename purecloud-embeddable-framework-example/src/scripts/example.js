document.addEventListener('DOMContentLoaded',function(){
    ...
    window.addEventListener("message", function(event) {
        ...
        var message = typeof event.data === 'string' ? JSON.parse(event.data): null;
        if(message){
            if(message.type == "screenPop"){
                ...
                sendContactSearch();
            } else if(message.type == "laPaga") {
                document.getElementById("accion").innerHTML =  message.data.data.cantidad;
            }
        }
    });
